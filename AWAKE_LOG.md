...
### Tasks Completed
- Done **Anomaly alerting** -- Added deterministic anomaly detection for session-to-session metrics (missing cumulative stats, low tests-per-module) and exposed it as `awake anomalies`.

### PRs
- PR #61 -- Anomaly alerting for session metrics

### Decisions
1. Implemented anomaly alerting as a thin deterministic layer over the existing `insights` parser rather than duplicating log parsing logic.
2. Focused v1 on high-signal, low-false-positive detectors suitable for CI and notifications; more statistical detectors can come later.
