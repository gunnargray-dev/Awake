...
Orchestrates the full flow: clone a repo, run Awake's existing analyzers
(health, complexity, security, dead code, coverage map, blame), compute
an overall score, and store everything in the leaderboard database.

This module is the bridge between Awake's analysis tools and the
leaderboard data layer. It imports from the parent ``src/`` package
to reuse the exact same analyzers that Awake runs on itself.
...
# Import Awake's existing analyzers from the parent src/ package.
# These are the same tools Awake uses on itself -- we reuse them on
# any repo we clone.
import sys

# Ensure the repo root's src/ is importable
_REPO_ROOT = Path(__file__).resolve().parent.parent
if str(_REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(_REPO_ROOT))

from src.health import HealthReport, analyze_health
from src.complexity import ComplexityReport, analyze_complexity
from src.security import SecurityReport, audit_security
from src.dead_code import DeadCodeReport, find_dead_code
...
from leaderboard.discovery import fetch_repo_metadata
